...
const wordSubject = document.getElementById('word-subject');
...
function resetVisualState() {
    ...
    wordSubject.classList.remove('active');
...
function updateVisuals(time) {
    ...
    let active = 0;
    if (time >= 80) active = 80;
    else if (time >= 70) active = 70;
    else if (time >= 65) active = 65;
    else if (time >= 57) active = 57;
    else if (time >= 41) active = 41;
    else if (time >= 28) active = 28;
    else if (time >= 15) active = 15;
    else if (time >= 8) active = 8;
    setActiveMarker(active);

    if (time >= 28 && time < 80) {
        wordSubject.classList.add('active');
    }
    ...
    if (time >= 15) {
        if (time < 65) {
            ...
            tableRows.b.classList.add('highlight-row');
            blankSpace.textContent = 'Compared';
...
function updateSubtitles(time) {
    ...
    if (activeIndex !== -1) {
        ...
        if (nextIdx !== -1) {
            ...
            subActive.textContent = "Bài giảng kết thúc. Hãy nhớ ôn tập rút gọn mệnh đề nhé!";
