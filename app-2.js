...
const wordPriorTo = document.getElementById('word-prior-to');
...
function resetVisualState() {
    ...
    wordPriorTo.classList.remove('active');
...
function updateVisuals(time) {
    ...
    let active = 0;
    if (time >= 91) active = 91;
    else if (time >= 80) active = 80;
    else if (time >= 64) active = 64;
    else if (time >= 56) active = 56;
    else if (time >= 43) active = 43;
    else if (time >= 28) active = 28;
    else if (time >= 17) active = 17;
    setActiveMarker(active);

    if (time >= 43 && time < 91) {
        wordPriorTo.classList.add('active');
    }
    ...
    if (time >= 28) {
        if (time < 64) {
            ...
            if (time < 76) {
                ...
                tableRows.c.classList.add('highlight-row');
                blankSpace.textContent = 'entering';
...
function updateSubtitles(time) {
    ...
    if (activeIndex !== -1) {
        ...
        if (nextIdx !== -1) {
            ...
            subActive.textContent = "Bài giảng kết thúc. Nhớ quy tắc giới từ nhé!";
