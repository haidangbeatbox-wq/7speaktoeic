...
const wordImperative = document.getElementById('word-imperative');
...
/*
 * Timeline state machine for Câu 1 (Subjunctive Mood)
 *
 * 0-13s:    Intro / đọc đề
 * 13-55s:   Nhận diện cú lừa — highlight "imperative", discuss A & D trap
 *   39-51s: Option A (is) & D (has been) discussed as trap
 *   51s:    A & D eliminated
 * 55-97s:   Giải thích Subjunctive mood — grammar card appears
 *   55s:    Option C (will be) implicitly wrong (no will in subjunctive)
 *   ~75s:   C eliminated
 * 97-111s:  Phá án — Option B confirmed
 *   107s:   B confirmed correct, blank filled
 * 111-end:  Từ vựng bỏ túi
 */
function updateVisuals(time) {
    ...
    let active = 0;
    if (time >= 139) active = 139;
    else if (time >= 111) active = 111;
    else if (time >= 97) active = 97;
    else if (time >= 55) active = 55;
    else if (time >= 13) active = 13;
    setActiveMarker(active);

    // Highlight "imperative" keyword when teacher discusses it (13s - 55s)
    if (time >= 13 && time < 97) {
        wordImperative.classList.add('active');
    }

    // Grammar card appears at 55s (subjunctive explanation)
    if (time >= 55) {
        grammarCard.classList.remove('hidden');
    }

    // Option A (is) - discussed at 39s, trapped, eliminated at 51s
    if (time >= 39) {
        if (time < 55) {
    ...
    // Option D (has been) - discussed alongside A, eliminated at 51s
    if (time >= 39) {
        if (time < 55) {
    ...
    // Option C (will be) - eliminated when subjunctive is explained (~75s)
    if (time >= 55) {
        if (time < 75) {
    ...
    // Option B (be) - discussed at 97s, confirmed correct at 107s
    if (time >= 97) {
        if (time < 107) {
...
function updateSubtitles(time) {
    ...
    if (activeIndex !== -1) {
        ...
        if (nextIdx !== -1) {
            ...
            subActive.textContent = "Bài giảng kết thúc. Nắm vững Subjunctive Mood nhé!";
...
Object.entries(optionCards).forEach(([key, card]) => {
    card.addEventListener('click', () => {
        let t = 0;
        if (key==='a') t=39; else if (key==='b') t=97; else if (key==='c') t=55; else if (key==='d') t=39;
        audio.currentTime = t; updateVisuals(t); updateSubtitles(t); togglePlayState(true);
    });
});
