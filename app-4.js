...
function updateVisuals(time) {
    ...
    let active = 0;
    if (time >= 73) active = 73;
    else if (time >= 61) active = 61;
    else if (time >= 53) active = 53;
    else if (time >= 46) active = 46;
    else if (time >= 40) active = 40;
    else if (time >= 28) active = 28;
    else if (time >= 12) active = 12;
    ...
    if (time >= 12) {
        if (time < 61) {
            ...
            tableRows.b.classList.add('highlight-row');
            blankSpace.textContent = 'renovated';
...
function updateSubtitles(time) {
    ...
    if (activeIndex !== -1) {
        ...
        if (nextIdx !== -1) {
            ...
            subActive.textContent = "Bài giảng kết thúc. Hãy nhớ ôn tập cấu trúc cầu khiến nhé!";
