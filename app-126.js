...
const wordHasBeen = document.getElementById('word-has-been');
...
function resetVisualState() {
    ...
    wordHasBeen.classList.remove('active');
...
/*
 * Timeline state machine for Câu 126
 * 0-7s:   Intro
 * 7-18s:  Focus on "has been negotiating". Highlight word-has-been.
 * 18-24s: Focus on "for the ... six weeks". Highlight blank-space.
 * 24-34s: Discuss A (past) -> confirmed correct at ~32s, fill blank.
 * 34-44s: Discuss B (former) -> eliminated at 43s.
 * 44-54s: Discuss C (remaining) -> eliminated at 53s.
 * 54-61s: Discuss D (final) -> eliminated at 61s.
 * 61-63s: Confirms A is correct again.
 * 63-end: Grammar tip. Show grammar card.
 */
function updateVisuals(time) {
    ...
    else if (time >= 45) active = 45;
    else if (time >= 35) active = 35;
    else if (time >= 25) active = 25;
    else if (time >= 20) active = 20;
    else if (time >= 0) active = 0;
    setActiveMarker(active);

    // Highlight "has been negotiating"
    if (time >= 7 && time < 63) {
        wordHasBeen.classList.add('active');
    }
    ...
    if (time >= 25 && time < 35) {
        ...
        if (time >= 32) {
            optionCards.a.classList.add('correct');
            blankSpace.textContent = 'past';
...
function updateSubtitles(time) {
    ...
    if (activeIndex !== -1) {
        ...
        if (nextIdx !== -1) {
            ...
            subActive.textContent = "Bài giảng kết thúc. Nhớ mẹo for the past + thời gian nhé!";
...
Object.entries(optionCards).forEach(([key, card]) => {
    card.addEventListener('click', () => {
        let t = 0;
        if (key==='a') t=25; else if (key==='b') t=35; else if (key==='c') t=45; else if (key==='d') t=55;
        audio.currentTime = t; updateVisuals(t); updateSubtitles(t); togglePlayState(true);
    });
});
