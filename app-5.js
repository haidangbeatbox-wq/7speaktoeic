...
function updateVisuals(time) {
    ...
    let active = 0;
    if (time >= 74) active = 74;
    else if (time >= 62) active = 62;
    else if (time >= 52) active = 52;
    else if (time >= 45) active = 45;
    else if (time >= 31) active = 31;
    else if (time >= 16) active = 16;
...
function updateSubtitles(time) {
    ...
    if (activeIndex !== -1) {
        ...
        if (nextIdx !== -1) {
            ...
            subActive.textContent = "Bài giảng kết thúc. Hãy nhớ ôn tập cụm along with này nhé!";
